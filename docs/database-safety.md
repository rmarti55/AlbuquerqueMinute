...
| `npm run sync:legistar` / `sync:meetings` | Normal ops | ABQ URL only |
| `npm run db:migrate-source-id -- --apply` | ALTER `source_id` to text | Host is `ep-blue-sky-*`; dry-run first |
